...
int main(void)
{
    ...
    while (1)
    {
        ...
        // select which camera is active
        if (g_ram_mux) {
            // This is (looking towards the front of the binocular camera
            // module, i.e. staring into the cameras) the left camera.
            DCMI_PWDN_LOW();
        }
        else {
            // This is (looking towards the front of the binocular camera
            // module, i.e. staring into the cameras) the right camera.
            DCMI_PWDN_HIGH();
        }
        usleep(10);
